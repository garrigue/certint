...
Lemma unify_keep_fv : forall K S E pairs K0 S0 HS0 HK0 h,
...
Proof.
  ...
  apply* (unify_ind (K':=K) (S':=S)
    ...
    clear H HS0 HK0 K0 S0 pairs h.
       intros until h.
       unfold K1, S1, fvs in *; clear K1 S1.
...
Definition unify_dep T1 T2 K S HS HK :
  let res :=  @unify ((T1,T2)::nil) K S HS HK (lt2_wf _) in
...
Proof.
  ...
  case_eq res; intros.
    ...
    do 2 rewrite* dom_remove_env.
    puts (typ_subst_res_fresh' _ HS H2).
    intro.
    destruct (x == v).
      right; subst. refine (proj2 (notin_union _ _ _) _); auto.
    destruct* (H6 x).
    destruct (x == v0). subst*.
    right; subst. refine (proj2 (notin_union _ _ _) _); auto.
   intros.
   ...
   replace (typ_fv T1 \u typ_fv T2) with (all_fv id ((T1,T2)::nil)).
    apply* unify_keep_fv.
   unfold all_fv; simpl. do 2 rewrite typ_subst_id. rewrite* union_empty_r.
  right*.
Qed.
...
Proof.
  ...
  revert Eb EB H.
  induction H0; intros.
    inversions H. simpl. split. apply ok_empty. split*. intro; auto*.
  simpl in H1.
  ...
  case_rewrite R2 (S.mem x B).
    ...
    case_eq (get x (e0 & Eb)); intros.
      elim (binds_fresh (in_ok_binds _ _ (I2 _ (binds_in H3)) H0) H).
    poses Hv' (get_none_notin _ H3); clear H3.
    split.
      apply* disjoint_ok.
      puts (ok_disjoint _ _ Hok).
      simpl.
      disjoint_solve.
      destruct* (x == v). subst*.
    split*.
    ...
    split; intro; simpl; intros.
      ...
      puts (I1 _ H3). destruct* (in_app_or _ _ _ H4).
  ...
  split.
    ...
    puts (ok_disjoint _ _ Hok).
    simpl.
    disjoint_solve; destruct* (x == v). subst*.
  split.
    simpl.
    disjoint_solve; destruct* (x == v); subst*.
  split*.
...
Proof.
  intros.
  rewrite dom_concat.
  apply disjoint_comm; apply disjoint_union; apply* disjoint_comm.
...
Proof.
  ...
  split.
    apply* disjoint_ok.
    assert (dom e4 << dom e).
      puts (incl_subset_dom I4). rewrite dom_concat in H0. auto.
    clear -H H0; disjoint_solve. right*.
  assert (incl (e0 & e4) K).
  ...
  split.
    clear -D' H1.
    disjoint_solve.
    assert (v # K). unfold K; rewrite* dom_map.
    right*.
  ...
  destruct (S.union_1 H3); clear H3; auto.
  ...
  destruct (S.union_1 H4); clear H4.
    unfold T1 in H2.
    puts (typ_fv_subst S' _ H2); clear -H3. simpl in H3. auto.
  puts (split_combine _ R4).
  rewrite <- (fv_in_kind_fv_list (l ++ B)) in H2.
    rewrite combine_app in H2.
      rewrite H3 in H2.
  ...
  rewrite (split_length _ R4). rewrite* map_length.
Qed.
...
Lemma disjoint_empty : forall L, disjoint L {}.
  intros; intro; auto.
Qed.
Hint Resolve disjoint_empty.
...
Lemma unify_type : forall K' S' pairs K S h HS HK ,
...
Proof.
  introv HU.
  apply* (unify_ind (K':=K') (S':=S') (fun K S pairs =>
    env_prop type S -> kenv_ok K ->
    (forall T1 T2, In (T1, T2) pairs -> type T1 /\ type T2) ->
    kenv_ok K' /\ env_prop type S' /\ is_subst S')); clear; intros.
        unfold S1, K1 in *.
        apply* H4; clear H4.
      ...
      destruct* (unify_kinds_all_kind_types (P:=type) H).
          ...
          apply (proj2 H6 _ _ (binds_in Bv)).
...
Lemma soundness_ind : forall h t K0 E T L0 S0 HS0 HK0 D0 HL0 K S L H' M x HK1,
  scheme M ->
  fresh L0 (sch_arity M) x ->
  @unify ((sch_open_vars M x,T)::nil) (K0 & kinds_open_vars (sch_kinds M) x)
    S0 HS0 HK1 (lt2_wf _) = Some (K, S) ->
...
Proof.
  ...
  refine (soundness_ind _ _ _ _ _ _ _ _ HI _ _ HI' _ _ _); auto.
...
Proof.
  ...
  rewrite sch_subst_fresh; try (unfold M; rewrite Delta.closed; intro; auto).
  split.
    sets_solve.
...
Proof.
  ...
  destruct (unify_type _ _ _ R1) as [HKK' HTS']; auto.
  ...
  destruct* (unify_kinds_ok _ _ _ R1) as [_ [_ WS']].
...
Proof.
  ...
  case_rewrite R1 (typinf0 t (typ_fvar var_default) is_subst_id 
           (ok_empty kind) (disjoint_empty (dom id))
  ...
  induction T. simpl*.
    case_eq (get v S0); intros.
      rewrite Hsub. rewrite Hsub.
        ...
        puts (binds_dom H). auto with sets.
...
Proof.
  ...
  replace (sch_kinds M')
    ...
    binds_cases H0.
      ...
      puts (fv_in_spec kind_fv _ _ _ (binds_in B)).
      apply* (fresh_disjoint (sch_arity M)).
      apply* fresh_sub.
    apply* well_kinded_subst.
...
Proof.
  ...
  clearbody HK0'.
  destruct* (unify_complete0 (K:=K) HS0 HK0' (lt2_wf _) Hext' HU)
  ...
  destruct (unify_dep (sch_open_vars M0 Xs) T HS0 HK0'
    ...
    [|rewrite HU1 in HU'; discriminate].
  esplit; esplit; esplit; esplit; split*.
  rewrite HU1 in HU'. inversions HU'; clear HU'.
  destruct* (unify_kinds_ok _ _ _ HU1).
    rewrite dom_concat.
    rewrite* dom_kinds_open_vars.
    puts (fresh_sub _ _ Fr HL). clear Fr.
    unfold fvs in H4; disjoint_solve.
  split.
    apply (typ_subst_extend _ _ _ HU1).
...
Proof.
  ...
  destruct* (unify_complete0 (K:=K) HS0 (proj1 HK0) (lt2_wf _) Hext' HU)
    as [K1 [S1 [HU1 [Hext1 WS1]]]].
  ...
  destruct (unify_type _ _ _ HU1) as [HKK' [HTS' _]]; auto.
    simpl; intros.
    destruct H; try contradiction.
    inversions H.
    split; auto. apply* (typ_subst_type' S).
  destruct* (unify_kinds_ok _ _ _ HU1) as [_ [_ WS']].
  poses Uk (unify_keep_fv (E:=E0) _ _ _ HU1).
  poses UT (unify_types _ _ _ HU1).
  ...
  assert (TUs: env_prop type (combine Xs Us)).
    ...
    destruct* H. inversion H. rewrite* <- H8.
  forward~ (@IH (L\u{{x1}}\u{{x2}}) S' K' (E0 & x ~ Sch (typ_fvar x1) nil)
  ...
  split.
    ...
    intuition. sets_solve.
      rewrite <- (S.singleton_1 H9) in *. auto.
    rewrite <- (S.singleton_1 H7) in *. auto.
  split*.
Qed.
...
Proof.
  ...
  assert (Oke1: kenv_ok (e2 & e1)).
    ...
    destruct* (split_env_ok _ R2).
    split*. intro; intuition. use (proj2 Oke).
  assert (HM0: scheme M0).
    ...
    apply list_forall_app.
      ...
      rewrite <- (split_combine _ R3) in H1.
      apply* env_prop_list_forall.
    unfold l0'; clear. induction Bs; simpl*.
  ...
  assert (Ok04: kenv_ok (e0 & e4)).
    split*.
    intro; intros. destruct OkK1; use (Inc04 _ H1).
  assert (OkE0': env_ok (E0 & x ~ M0)) by split*.
  ...
  forward~ (IH L' S' (e0&e4) (E0&x~M0) HS' Ok04 Dis04
       ...
       binds_cases H1.
        ...
        rewrite <- union_assoc.
        intro; intros. destruct* (S.union_1 H1).
       auto*.
      repeat rewrite dom_concat in *. intuition.
      sets_solve. simpl in H10. auto.
      intuition.
     intro; intros.
     ...
     assert (x0 \in env_fv E \u fv_in kind_fv K).
       assert (Z \in ftve). apply (proj42 H0). apply (binds_dom B).
         ...
         intros; rewrite* Hsub'.
       destruct* (fresh_disjoint _ _ _ Fr x0). elim H10; auto.
     destruct* (split_env_ok _ R4). destruct* Oke.
     ...
     rewrite Hsub' in H6; [|simpl*]. rewrite Hsub in H6; [|simpl*].
     subst. puts (WS _ _ H13).
     rewrite <- H6 in H15.
     inversions H15.
     clear -H7 H23 Typ1 Fr.
     destruct* (ok_disjoint _ _ (proj1 (proj41 (typing_regular Typ1))) x0).
       elim (H (binds_dom H23)).
     rewrite dom_kinds_open_vars in H. elim (H H7). auto.
...
Proof.
  ...
  forward~ (IH L' S' K' E0 HS' HK' D' (S & x1 ~ S1 & S'') K E t2 (typ_fvar x1)).
     ...
     repeat rewrite dom_concat; simpl. sets_solve. apply* HL'.
...
Proof.
  ...
  assert (WS': well_subst (K0 & kinds_open_vars (sch_kinds M) Xs) K 
    ...
    rewrite map_length. rewrite* (fresh_length _ _ _ Fr).
    rewrite* <- (fresh_length _ _ _ Fr).
  destruct* (unify_complete0 (K:=K) HS0 Ok (lt2_wf _) Hext' HU)
    as [K1 [S1 [HU1 [Hext1 WS1]]]].
...
Proof.
  ...
  forward~
    (@typinf_principal (S (trm_depth t)) (env_fv E \u {{var_default}})
      id empty E is_subst_id HK0 (disjoint_empty (dom id))
  ...
  subst. left; intro. elim (S.diff_2 (H _ H2)). auto.
Qed.
